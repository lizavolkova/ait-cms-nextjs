...
export default function ImageGallery({ element }) {
    ...
    useEffect(() => {
        ...
        const doSomethingAsync = async item => {
            const url = await fetch(item.src).then(data => data.url);
            return {
                sourceUrl: url,
                mediaDetails: {
                    height: item.height,
        ...
        const test = async () => {
            return Promise.all(photosJson.map(photo => doSomethingAsync(photo)))
        }
                                ...
                                <Link href="gallery/outlander">
                                    <>
                                        ...
                                        <div className="absolute transition-opacity duration-300 inset-x-0 bottom-0 z-30 opacity-100 group-hover:opacity-100 lg:opacity-0">
                                            ...
                                            {photo && <CoverImage title="" coverImage={photo} slug="" />}
                                    </>
                                </Link>
                            </li>
                        )
