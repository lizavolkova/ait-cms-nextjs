...
export async function getAllFeaturedCostumes() {
  const data = await fetchAPI(`
  {
     costumes(where: {orderby: {field: DATE, order: DESC}}) {
      nodes {
        id
        costumeId
        title
...
export async function getCostumeBySlug(slug) {
  const data = await fetchAPI(`
  {  
    costume(id: "${slug}", idType: SLUG) {
        slug
        title
        id
        date
...
export async function getAllPostsByCategory(preview, category) {
  const data = await fetchAPI(
    `
    query AllPosts {
